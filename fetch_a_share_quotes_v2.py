...
def main():
    print("=" * 80)
    print(
        f"A股实时行情获取 - 多数据源备份版 ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})"
    )
    print("=" * 80)
    ...
    for q in quotes:
        print(
            f"{q['code']:<12} {q['name']:<10} {q['price']:>10.2f} {q['change_pct']:>7.2f}% {q['high']:>10.2f} {q['low']:>10.2f}"
        )
    ...
    output_file = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "data", "a_share_quotes_today.json"
    )
