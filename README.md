...
- **Slack推送**：支持多客户配置，一键群发或指定客户发送
- **Web 仪表盘**：6个页面，无需部署，浏览器即可查看完整分析
...
| 页面 | 功能 |
|------|------|
| **市场总览** | 沪深300实时行情(日K线) + 市场情绪 + 市场状态 + 仓位建议阈值 |
| **沪深300** | 300只成分股综合评分排名，搜索/筛选/搜索，触发重新分析（进度条） |
| **行业板块** | 28行业20日/5日动量排行（柱状图 + 表格） |
| **我的组合** | 持仓增删改 + 盈亏计算 + 行业分布饼图 + 每只股票买卖建议 |
| **说明** | 系统使用说明和评分体系详解 |
| **设置** | 多客户 Slack 配置，支持指定发送或一键群发 |

沪深300页面还支持 **生成报告**（预览完整分析文本）和 **发送到 Slack**（选客户或群发）。
...
│   └── utils/
│       └── utils.py        # 工具函数
├── .python-version         # Python 版本约束 (uv 使用)
├── backend/                # FastAPI 后端
│   ├── main.py             # 入口 + CORS + 静态文件托管
│   ├── schemas.py          # Pydantic 数据模型
│   └── routers/
│       ├── market.py       # /api/market/overview, /history
│       ├── analysis.py     # /api/analysis/daily, /report, /refresh, /status, /send-slack
│       ├── sectors.py      # /api/sectors
│       ├── portfolio.py    # /api/portfolio (CRUD + analyze)
│       ├── settings.py     # /api/settings/slack (多配置 CRUD)
│       └── common.py       # 共享工具 (patch_results, PROJECT_ROOT)
├── frontend/               # React 前端
│   ├── src/
...
│   │   └── pages/
│   │       ├── Dashboard.tsx   # 市场总览
│   │       ├── Stocks.tsx      # 沪深300成分股 + 报告 + Slack发送
│   │       ├── Sectors.tsx     # 行业板块
│   │       ├── Portfolio.tsx   # 我的组合
│   │       ├── About.tsx       # 使用说明
│   │       └── Settings.tsx    # Slack 多客户配置
│   └── package.json
├── config/
...
- `data/raw/slack_configs.json` - Slack 多客户配置
- `data/raw/analysis_status.json` - 分析任务状态

**提交到 Git 的环境配置文件**：
- `.python-version` - 指定 Python 3.11，uv 自动使用
...
**系统要求**：
- Python 3.9+
- Node.js 18+（仅前端构建需要，运行时只需要 dist/）
- 4GB+ 内存
- 稳定网络连接（需要访问腾讯/新浪财经 API）
